...
public final class RatPoly {
     ...
     * @requires e >= 0
     * @effects Constructs a new Poly equal to "c*x^e". If c is zero, constructs a
     *          "0" polynomial.
     */
     ...
     * @effects Constructs a new Poly using 'rt' as part of the representation. The
     *          method does not make a copy of 'rt'.
     */
     ...
     * @requires !this.isNaN()
     * @return the RatTerm of degree 'deg'. If there is no term of degree 'deg' in
     *         this poly, then returns the zero RatTerm.
     */
    ...
    /**
     * Helper procedure: Inserts a term into a sorted sequence of terms, preserving
     * the sorted nature of the sequence. If a term with the given degree already
     * exists, adds their coefficients.
     *
     * Definitions: Let a "Sorted List<RatTerm>" be a List<RatTerm> V such that [1]
     * V is sorted in descending exponent order && [2] there are no two RatTerms
     * with the same exponent in V && [3] there is no RatTerm in V with a
     * coefficient equal to zero
     *
     * For a Sorted List<RatTerm> V and integer e, let cofind(V, e) be either the
     * coefficient for a RatTerm rt in V whose exponent is e, or zero if there does
     * not exist any such RatTerm in V. (This is like the coeff function of
     * RatPoly.) We will write sorted(lst) to denote that lst is a Sorted
     * List<RatTerm>, as defined above.
     *
     * @param lst
     ...
     *
     * @return a RatPoly equal to "0 - this"; if this.isNaN(), returns some r such
     *         that r.isNaN()
     */
     ...
     * @requires p != null
     * @return a RatPoly, r, such that r = "this + p"; if this.isNaN() or p.isNaN(),
     *         returns some r such that r.isNaN()
     */
    public RatPoly add(RatPoly p) {
        ...
        if (this.isNaN() || p.isNaN()) {
            ...
            for (int i = 0; i < this.terms.size(); i++) {
                sortedInsert(tmp, this.terms.get(i));
            }
            return new RatPoly(tmp);
        }
    }
    ...
    public RatPoly mul(RatPoly p) {
        ...
        if (this.isNaN() || p.isNaN()) {
            ...
            RatPoly tmp = new RatPoly();
            for (int i = 0; i < this.terms.size(); i++) {
                List<RatTerm> tmpList = new ArrayList<>(p.terms);
                scaleCoeff(tmpList, terms.get(i).getCoeff());
                incremExpt(tmpList, terms.get(i).getExpt());
                tmp = tmp.add(new RatPoly(tmpList));
            }
            return tmp;
        }
     ...
     *            The divisor.
     * @requires p != null
     * @return a RatPoly, q, such that q = "this / p"; if p = 0 or this.isNaN() or
     *         p.isNaN(), returns some q such that q.isNaN()
     *         <p>
     *
     *         Division of polynomials is defined as follows: Given two polynomials
     *         u and v, with v != "0", we can divide u by v to obtain a quotient
     *         polynomial q and a remainder polynomial r satisfying the condition u
     *         = "q * v + r", where the degree of r is strictly less than the degree
     *         of v, the degree of q is no greater than the degree of u, and r and q
    ...
    public RatPoly div(RatPoly p) {
        ...
        if (p.equals(ZERO) || this.isNaN() || p.isNaN()) {
            ...
            List<RatTerm> tmp = new ArrayList<>(this.terms);
            RatPoly poly1 = new RatPoly();
            RatPoly poly2 = new RatPoly(tmp);

            while (poly2.terms.size() != 0 && p.degree() <= poly2.degree()) {
                RatNum tmpNum = poly2.terms.get(0).getCoeff().div(p.terms.get(0).getCoeff());
                RatPoly tmpPoly = new RatPoly(new RatTerm(tmpNum, poly2.degree() - p.degree()));
                poly1 = poly1.add(tmpPoly);
                poly2 = poly2.sub(tmpPoly.mul(p));
            }
            return poly1;
        }
    }
     ...
     *
     * @return a RatPoly, q, such that q = dy/dx, where this == y. In other words, q
     *         is the derivative of this. If this.isNaN(), then return some q such
     *         that q.isNaN()
     *
     *         <p>
     *         The derivative of a polynomial is the sum of the derivative of each
     *         term.
     */
    public RatPoly differentiate() {
        ...
        if (this.isNaN()) {
            ...
            for (int i = 0; i < tmp.size(); i++) {
                if (tmp.get(i).getExpt() != 0) {
                    tmp.set(i, tmp.get(i).differentiate());
                } else {
                    tmp.remove(i);
                }
            }
     ...
     * @return a RatPoly, q, such that dq/dx = this and the constant of integration
     *         is "integrationConstant" In other words, q is the antiderivative of
     *         this. If this.isNaN() or integrationConstant.isNaN(), then return
     *         some q such that q.isNaN()
     *
     *         <p>
     *         The antiderivative of a polynomial is the sum of the antiderivative
     *         of each term plus some constant.
     */
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        ...
        if (this.isNaN() || integrationConstant.isNaN()) {
            ...
            List<RatTerm> tmp = new ArrayList<>(terms);
            for (int i = 0; i < tmp.size(); i++) {
                tmp.set(i, tmp.get(i).antiDifferentiate());
            }
            if (!integrationConstant.equals(RatNum.ZERO)) {
                tmp.add(new RatTerm(integrationConstant, 0));
            }
            return new RatPoly(tmp);
        }
    }
     ...
     *
     * <p>
     * The Fundamental Theorem of Calculus states that the definite integral of f(x)
     * with bounds a to b is F(b) - F(a) where dF/dx = f(x) NOTE: Remember that the
     * lowerBound can be higher than the upperBound.
     *
     * @param lowerBound
     ...
     *         integration between lowerBound and upperBound. If this.isNaN(), or
     *         either lowerBound or upperBound is Double.NaN, return Double.NaN.
     */
    public double integrate(double lowerBound, double upperBound) {
        ...
        if (this.isNaN() || Double.isNaN(lowerBound) || Double.isNaN(upperBound)) {
            ...
            RatPoly tmp = antiDifferentiate(new RatNum(1, 1));
            return tmp.eval(upperBound) - tmp.eval(lowerBound);
        }
     ...
     *
     * @return A String representation of the expression represented by this, with
     *         the terms sorted in order of degree from highest to lowest.
     *         <p>
     *         There is no whitespace in the returned string.
     *         <p>
     *         If the polynomial is itself zero, the returned string will just be
     *         "0".
     *         <p>
     *         If this.isNaN(), then the returned string will be just "NaN"
     *         <p>
     *         The string for a non-zero, non-NaN poly is in the form
     *         "(-)T(+|-)T(+|-)...", where "(-)" refers to a possible minus sign, if
     *         needed, and "(+|-)" refer to either a plus or minus sign, as needed.
     *         For each term, T takes the form "C*x^E" or "C*x" where C > 0, UNLESS:
     *         (1) the exponent E is zero, in which case T takes the form "C", or
     *         (2) the coefficient C is one, in which case T takes the form "x^E" or
     *         "x". In cases were both (1) and (2) apply, (1) is used.
     *         <p>
     *         Valid example outputs include "x^17-3/2*x^2+1", "-x+1", "-1/2", and
     *         "0".
     *         <p>
     */
    @Override
     ...
     * @requires 'polyStr' is an instance of a string with no spaces that expresses
     *           a poly in the form defined in the toString() method, except that
     ...
     * @return true if and only if 'obj' is an instance of a RatPoly and 'this' and
     *         'obj' represent the same rational polynomial. Note that all NaN
     *         RatPolys are equal.
     */
    @Override
